...
import {
    UploaderContext,
    useFilesUploader,
} from './helpers/useFilesUploader.ts';
        ...
        const uploadMediaFiles = async () => {
            ...
            selectedFiles.value
                ...
                .map(async (file) => {
                    ...
                    dispatch(
                        setBrandSettingsByField({ field: 'images', newData })
                    );
                });
        };
    ...
    return (
        ...
        >
            <div className={className}>
                {children && React.cloneElement(children, { id: name })}
            </div>
        </UploaderContext.Provider>
    );
};
...
FilesUploader.Input = function Input({
    label = '+ Загрузить',
    ...props
}: IFilesUploaderProps) {
    ...
    return (
        <div id={uid(id)}>
            ...
            />
            <label htmlFor={`file-uploader-input-button-${id}`}>{label}</label>
        </div>
    );
};
