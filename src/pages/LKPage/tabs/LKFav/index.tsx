  ...
  return (
    <div className={cx.container}>
      <div className={cx.fav}>
        ...
        {
          mockData1.length > 0
            ? <div className={cx.list}>{mockData1.map((_, index) => <ProductItem key={uid(index)} size="apativeSize" />)}</div>
            : <EmptyList text="кажется, вам пока ничего не понравилось" 
      ...
      <div className={cx.relevant}>
        ...
        {
          mockData2.length > 0
            ? <div className={cx.list}>{mockData2.map((_, index) => <ProductItem key={uid(index)} size="apativeSize" />)}</div>
            : <EmptyList text="нет рекомендаций для вас" 
                btnText="отправиться на поиски" 
                link={ERoutes.Default} 
              />
        }
