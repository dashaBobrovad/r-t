...
export default function SchemePage() {
    ...
    const { data, error, isLoading } = useGetBrandPageStyleDataQuery({
        scheme_id: +schemeId,
        vendor_id: vendorIdReturner(+schemeId),
    });

    const [dataObj, setDataObj] = useState<
        IScheme1Brand | IScheme2Brand | IScheme3Brand
    >();
    ...
    return (
        <div
            style={{
                background: dataObj?.background_color
                    ? dataObj.background_color
                    : 'inherit',
            }}
        >
            {error ? (
